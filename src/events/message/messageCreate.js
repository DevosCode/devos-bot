...
module.exports = async (client, message) => {  
  if (!message.author.bot) { 
    ...
    if (userDB.experience > xpObjectif) {
      ...
      if(parseInt(userDB.level) % 10 === 0 ){
        message.channel.send(`Tu viens d'augmenter de role Level ${userDB.level} +`)
        await message.member.roles.add(message.guild.roles.cache.find(r => r.name === `Level ${userDB.level} +`))
        await message.member.roles.remove(message.guild.roles.cache.find(r => r.name === `Level ${userDB.level - 10} +`))
      } else {
        console.log(userDB.level)
        console.log(userDB.level % 5)
      }
    }
