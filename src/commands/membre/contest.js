...
module.exports = {
  ...
  async run({ client, interaction }) {
    ...
    let findRole= false;
    for (const staff_role_id of await getSettingGroup(interaction.guild.id, "CONTEST_MANAGER_ROLES")) {
      if (!!findRole==false){
        const staff_role = interaction.guild.roles.cache.find(r => r.id == staff_role_id); 
        if (staff_role) {
          findRole = true;
        }
      }
      ...
      if (findRole==false) {
        return error(interaction, "Tu ne peux pas créer de contest.");
      }
